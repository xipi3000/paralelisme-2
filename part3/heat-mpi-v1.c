...
if (myid == 0) {
    ...
    unsigned divided_value = param.resolution / numprocs;
    // starting time
    runtime = wtime();
    ...
    for (int i=0; i<numprocs; i++) {
        if (i>0) {
                ...
                if(i==numprocs-1){
                    unsigned divided_value_last = param.resolution - param.resolution/numprocs * i;
                    MPI_Send(&divided_value_last, 1, MPI_INT, i, 0, MPI_COMM_WORLD);
                }
                else
                {
                    MPI_Send(&divided_value, 1, MPI_INT, i, 0, MPI_COMM_WORLD);
                }
                MPI_Send(&param.algorithm, 1, MPI_INT, i, 0, MPI_COMM_WORLD);
                if(i==numprocs-1){
                    MPI_Send(&param.u[i*((np)*(np))/numprocs], (np)*(np)-(i*((np)*(np))/numprocs), MPI_DOUBLE, i, 0, MPI_COMM_WORLD);
                }else
                {
                    MPI_Send(&param.u[i*((np)*(np))/numprocs], ((np)*(np))/numprocs, MPI_DOUBLE, i, 0, MPI_COMM_WORLD);
                }
